    ...
    <div className="w-full h-full flex flex-col pointer-events-auto">
        ...
        <search className="flex-1 flex justify-start lg:justify-center w-full lg:w-auto pointer-events-auto">
          <div className="flex items-center gap-3 bg-stone-900/60 backdrop-blur-md border border-white/10 p-2 md:p-3 rounded-2xl w-full max-w-lg md:max-w-none md:w-auto">
            <div className="flex items-center gap-2 px-2 text-stone-400">
              <CalendarIcon className="w-4 h-4 hidden md:block" aria-hidden="true" />
            ...
            <div className="flex items-center gap-2 flex-1 md:flex-initial">
              ...
              <ChevronRight className="w-4 h-4 text-stone-600 shrink-0" aria-hidden="true" />
      ...
      <div className="flex-1 overflow-y-auto w-full custom-scrollbar pointer-events-auto">
        <div className="max-w-7xl mx-auto p-6 md:p-8">
          <section className="animate-in fade-in zoom-in-95 duration-500" aria-labelledby="timeline-heading">
            <h2 id="timeline-heading" className="text-lg font-mono text-amber-300/60 mb-6 flex items-center gap-2">
              <Clock className="w-4 h-4" aria-hidden="true" /> Timeline
            </h2>

            {loading ? (
              <p className="text-center py-20 text-stone-500 font-mono animate-pulse" role="status">Loading timeline...</p>
            ) : dates.length === 0 ? (
              <div className="text-center py-20">
                <div className="w-24 h-24 bg-stone-900/50 rounded-full flex items-center justify-center mx-auto mb-6">
                  <Clock className="w-10 h-10 text-stone-700" aria-hidden="true" />
                </div>
              ...
              <>
                <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {dates.slice((page - 1) * pageSize, page * pageSize).map((d) => (
                    <li key={d.date}>
                      ...
                      >
                        <span className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity" aria-hidden="true" />
                ...
                {totalPages > 1 && (
                  <nav className="mt-12 flex items-center justify-center gap-4 pb-8" aria-label="Timeline pagination">
                    <Button
                      variant="ghost"
                    ...
                    <span className="text-stone-400 font-mono text-sm" aria-live="polite">
