...
beforeEach(async () => {
  sessionDir = await fs.mkdtemp(join(tmpdir(), "pi-session-browser-"));
...
describe("Pi session storage", () => {
  it("lists sessions with metadata and orders them by their latest entry", async () => {
    const older = await writeSession("project/older.jsonl", [
      { type: "session", id: "older", cwd: "/work/older", timestamp: "2026-01-01T10:00:00.000Z" },
      { type: "message", timestamp: "2026-01-01T10:01:00.000Z", message: { role: "user", content: "First request" } },
    ]);
    const newer = await writeSession("project/newer.jsonl", [
      { type: "session", id: "newer", cwd: "/work/newer", timestamp: "2026-01-01T11:00:00.000Z" },
      { type: "session_info", name: "Named session" },
      { type: "message", timestamp: "2026-01-01T12:00:00.000Z", message: { role: "user", content: [{ type: "text", text: "A detailed request" }] } },
    ]);
  ...
  it("returns displayable conversation entries and ignores hidden custom messages", async () => {
    const file = await writeSession("conversation.jsonl", [
      { type: "session", id: "conversation", cwd: "/work", timestamp: "2026-01-01T10:00:00.000Z" },
      { type: "message", id: "user-1", timestamp: "2026-01-01T10:01:00.000Z", message: { role: "user", content: [{ type: "text", text: "Hello" }, { type: "image" }] } },
      { type: "message", id: "hidden", message: { role: "custom", content: "Do not show" } },
      { type: "compaction", id: "compact-1", timestamp: "2026-01-01T10:02:00.000Z", summary: "Earlier messages were compacted" },
      { type: "branch_summary", id: "branch-1", timestamp: "2026-01-01T10:03:00.000Z" },
    ]);
    ...
    await expect(sessions.getConversation(file)).resolves.toEqual({
      ...
      items: [
        { id: "user-1", timestamp: "2026-01-01T10:01:00.000Z", role: "user", text: "Hello\n[image]", toolName: undefined },
        { id: "compact-1", timestamp: "2026-01-01T10:02:00.000Z", role: "summary", text: "Earlier messages were compacted" },
        { id: "branch-1", timestamp: "2026-01-01T10:03:00.000Z", role: "summary", text: "Branch summary" },
      ],
    });
  });
  ...
  it("only accepts JSONL files contained in the configured session directory", async () => {
    ...
    await expect(sessions.safeSessionPath(outside)).rejects.toThrow("outside Pi's session directory");
