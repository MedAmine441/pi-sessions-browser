...
const sessionRoot = resolve(process.env.PI_SESSION_DIR || `${homedir()}/.pi/agent/sessions`);
const terminal = process.env.PI_TERMINAL || "x-terminal-emulator";
const piCommand = process.env.PI_COMMAND || "pi";
const maxSessionBytes = Number(process.env.PI_SESSION_BROWSER_MAX_BYTES || 100 * 1024 * 1024);
...
async function walk(directory: string): Promise<string[]> {
  ...
  for (const entry of entries) {
    ...
    else if (entry.isFile() && extname(entry.name) === ".jsonl") results.push(file);
  }
  return results;
}
...
export async function safeSessionPath(candidate: string | null) {
  ...
  if (typeof candidate !== "string" || !candidate) throw new Error("A session path is required.");
  const real = await fs.realpath(resolve(candidate));
  if (!(real === canonicalRoot || real.startsWith(`${canonicalRoot}${sep}`)) || extname(real) !== ".jsonl") {
...
async function load(file: string) {
  ...
  if (stat.size > maxSessionBytes) throw new Error(`Session is larger than the ${Math.round(maxSessionBytes / 1024 / 1024)} MB safety limit.`);
  const raw = await fs.readFile(file, "utf8");
  const entries = [];
  for (const line of raw.split("\n")) {
    if (!line.trim()) continue;
    try { entries.push(JSON.parse(line)); } catch { /* A partially-written final JSONL line is safe to ignore. */ }
  }
  return { stat, entries };
}
  ...
  for (const file of files) {
    ...
    if (match) {
      try {
        ...
        const timePart = match[1].substring(10).replace(/-/g, ":").replace(/:(\d{3}Z)$/, ".$1");
...
export async function listSessions(targetDate?: string, location?: string) {
  ...
  return loaded.filter(Boolean).sort((a: any, b: any) => Date.parse(b.updatedAt) - Date.parse(a.updatedAt));
}
...
export async function getConversation(file: string) {
  ...
  for (const entry of entries) {
    if (entry.type === "message") {
      ...
      items.push({ id: entry.id, timestamp: entry.timestamp || message.timestamp, role: message.role || "message", text: textFrom(message.content), toolName: message.toolName });
    } else if (entry.type === "compaction") {
      items.push({ id: entry.id, timestamp: entry.timestamp, role: "summary", text: entry.summary || "Conversation compacted" });
    } else if (entry.type === "branch_summary") {
      items.push({ id: entry.id, timestamp: entry.timestamp, role: "summary", text: entry.summary || "Branch summary" });
    }
  }
  const firstUserMsg = entries.find((e) => e.type === "message" && e.message?.role === "user")?.message?.content;
  let preview = "";
  if (firstUserMsg) {
    preview = short(textFrom(firstUserMsg) || "");
  }

  return { file, id: header.id, name: name || null, cwd: header.cwd, createdAt: header.timestamp, items, preview };
}

export async function launchSession(file: string) {
  const { entries } = await load(file);
  const cwd = entries.find((entry) => entry.type === "session")?.cwd;
  if (!cwd || typeof cwd !== "string") throw new Error("The session has no working directory.");
  
  // Values are passed as positional bash arguments, never interpolated into shell code.
  const script = 'cd -- "$1" || exit; exec "$2" --session "$3"';
  const child = spawn(terminal, ["-e", "bash", "-lc", script, "pi-session-browser", cwd, piCommand, file], {
    detached: true,
    stdio: "ignore",
    env: process.env,
  });
  child.once("error", (error) => console.error(`Could not launch ${terminal}: ${error.message}`));
  child.unref();
}
...
export async function launchNewSession(targetCwd?: string) {
  ...
  const child = spawn(terminal, ["-e", "bash", "-lc", script, "pi-session-browser", workingDir, piCommand], {
    detached: true,
    stdio: "ignore",
    env: process.env,
  });
...
export async function renameSession(file: string, newName: string) {
  const entry = { type: "name", id: randomUUID(), name: newName, timestamp: new Date().toISOString() };
  await fs.appendFile(file, JSON.stringify(entry) + "\n", "utf-8");
}

export async function editMessage(file: string, messageId: string, newText: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    try {
      ...
      if (parsed.type === "message" && parsed.id === messageId && parsed.message) {
        // Find text content and edit it
        if (Array.isArray(parsed.message.content)) {
          const textItem = parsed.message.content.find((c: any) => c.type === "text");
          if (textItem) {
